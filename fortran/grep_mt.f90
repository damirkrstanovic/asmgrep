! fortgrep_std_mt - idiomatic + NAIVE OpenMP threads. Collects the file list
! (recursive walk via POSIX opendir/readdir/lstat), then an `!$omp parallel do
! schedule(dynamic)` over the files. Each iteration reads the file IN FULL into
! a FRESH allocatable array before the binary check (deliberately allocation-
! heavy tier). Per-file output block serialized in an `!$omp critical`.
module posix_walk
  use iso_c_binding
  implicit none
...
contains
  ...
  ! NAIVE: each call allocates a fresh `data` array (no buffer reuse).
  ! Output assembled into a local buffer, flushed under !$omp critical.
  subroutine search_file(path)
    character(len=*), intent(in) :: path
    character(len=:), allocatable :: data, hay, needle, pfx, obuf
    integer(c_int64_t) :: fsize
    integer :: u, ios, n, peek, i, m, ls, le, pos, nlen, rel, olen, ocap
    ...
    if (ios /= 0) return
    allocate(character(len=n) :: data)          ! FRESH full read every file
    ...
    peek = min(n, PREFIX)
    do i = 1, peek
      if (data(i:i) == achar(0)) return         ! binary skip (after full read)
    end do
    ...
    allocate(character(len=max(n,64)) :: obuf)
    ocap = len(obuf)
    olen = 0
    local_match = .false.
    ...
    do while (pos <= n)
      ...
      local_match = .true.
      if (g_multi) call buf_append(obuf, ocap, olen, pfx)
      if (le > ls) call buf_append(obuf, ocap, olen, data(ls:le-1))
      call buf_append(obuf, ocap, olen, achar(10))
      pos = le + 1
    end do
    ...
    if (local_match) then
      ...
      g_matched = .true.
      if (olen > 0) write(g_out_unit) obuf(1:olen)
...
program grep_mt
  ...
  ! NAIVE parallel-do over the file list; no per-thread buffer reuse.
  !$omp parallel do schedule(dynamic) num_threads(nt)
  do i = 1, g_nfiles
    call search_file(trim(g_files(i)))
  end do
  !$omp end parallel do
